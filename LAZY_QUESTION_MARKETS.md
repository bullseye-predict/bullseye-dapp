...
Titles have separate jobs. `presentation.eventTitle` names the event once at page level. A linked answer uses `presentation.answer.label` (and optional image) instead of repeating the full binary question. A head-to-head market is labelled by its line name — `Moneyline` — rather than by its question sentence, which restates the event title it sits under; `marketLineTitle()` in `src/components/markets/moneyline.ts` is the single source of that name. The trade ticket's kicker is `presentation.eventTitle` for every presentation kind, so the ticket and the page H1 always agree. Charts inside event detail do not repeat the event title.
