...
export function questionFromArena(raw: any, agents: any, eventId: string, agentId: string, gameOrigin: string, now = Date.now()): GameQuestion {
  ...
  return { eventId, roomId: match.roomId, agentId, subjectId: participant.actorId, label: `Will ${agent.codename} win?`,
    sourceUrl: new URL(`/api/v1/agent-arena/matches/${encodeURIComponent(match.roomId)}/logs`, gameOrigin).toString(),
    tradingStartsAt: Math.floor(now / 1000) * 1000, tradingLocksAt,
