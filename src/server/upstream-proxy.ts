...
function predictionOrigin(runtime: RuntimeEnv) {
  return String(
    ...
    (typeof process !== 'undefined' ? process.env.PUBLIC_PREDICTION_API_URL : undefined) ??
...
function validOrigin(value: string) {
  try {
    ...
    return !url.username && !url.password && !url.pathname.replaceAll('/', '') && !url.search && !url.hash &&
...
export async function proxyPrediction(request: Request, path: string | undefined, runtime: RuntimeEnv, fetcher: Fetcher = fetch) {
  const origin = predictionOrigin(runtime)
  if (!validOrigin(origin)) return Response.json({ error: 'Prediction API origin is invalid.' }, { status: 503 })
  ...
  try {
    ...
    return Response.json({ error: 'Prediction API is temporarily unavailable.' }, { status: 502, headers: { 'cache-control': 'no-store' } })
  }
}
