import { MAX_BATCH, resolveTraderProfiles } from '../pump-profiles.ts'
 ...
 */
export async function postIdentityProfiles(request: Request, runtimeEnv: Record<string, unknown> = {}) {
  let payload: unknown
  ...
  const batch = await resolveTraderProfiles(addresses as string[], runtimeEnv)
...
export const rejectIdentityProfilesMethod = () =>
