import { directiveUnavailableReason, parseDirectiveSettings } from '../../components/solz/directiveRelay.ts'
...
function configuredColyseusUrl(runtimeEnv: Record<string, unknown> = {}) {
  return String(
    ...
    runtimeEnv.VITE_COLYSEUS_SERVER_URL ??
    runtimeEnv.VITE_COLYSEUS_SERVER_URL_ASIA ??
    ''
  ).trim().replace(/\/+$/, '')
}
...
function configuredGameOrigin(runtimeEnv: Record<string, unknown> = {}) {
  return String(runtimeEnv.SOLZ_GAME_ORIGIN ??
    ...
    runtimeEnv.VITE_SOLZ_GAME_ORIGIN ?? '')
    .trim()
    .replace(/\/+$/, '')
}
...
export async function getOverview(runtimeEnv: Record<string, unknown> = {}) {
