...
import { profileHref, sameProfileAddress, type ProfileRoute } from './profileRoute'
...
export function Portfolio({ apiUrl, wallet, walletControl, matchApiUrl = '', profile }: { apiUrl: string; matchApiUrl?: string; wallet: DynamicEvmWalletPort | null; walletControl: ReactNode; profile?: ProfileRoute }) {
  ...
  const owner = profile?.address ?? wallet?.address
  const isSelf = Boolean(wallet && owner && profile?.chain !== 'solana' && sameProfileAddress(wallet.address, owner))
  const canManage = isSelf && profile?.chain !== 'solana'
  const data = usePortfolio(apiUrl, chain, profile?.chain === 'solana' ? undefined : owner, retry, matchApiUrl)
  const scope = `${chain}:${owner?.toLowerCase()}`
  ...
  useEffect(() => {
    if (!profile && wallet) window.location.replace(profileHref('somnia', 'testnet', wallet.address))
  }, [profile, wallet])
  ...
  return <div className="solz-home pf-page">
    ...
    <main id="portfolio" className="pf-main">
      <div className="pf-heading"><div><p>{isSelf ? 'YOUR PROFILE' : owner ? 'PUBLIC PROFILE' : 'YOUR ACCOUNT'}</p><h1>{isSelf ? 'My portfolio' : 'Portfolio'}<span>↗</span></h1></div><label className="pf-network">Network<select value={chain} disabled={profile?.chain === 'solana'} onChange={e => selectNetwork(e.target.value as typeof chain)}><option value="50312">Somnia testnet · tUSDC</option><option value="5031">Somnia mainnet · USDso</option></select></label></div>
      <section className="pf-overview" aria-label="Account overview">
        <div className="pf-identity"><div className="pf-avatar"><WalletCards size={26}/></div><div><h2>{displayName}</h2><p>{profile?.chain === 'solana' ? 'Solana public profile · prediction positions are not configured for this network.' : isSelf ? 'Connected wallet · DreamDEX predictions' : owner ? 'Public Somnia prediction profile · read-only' : 'Connect your Somnia wallet to view and manage your positions.'}</p>{owner && <button className="pf-copy" onClick={() => void navigator.clipboard.writeText(owner).then(() => setCopyStatus('Address copied')).catch(() => setCopyStatus('Could not copy address'))}><Copy size={13}/>Copy address</button>}<span role="status">{copyStatus}</span></div></div>
        <dl className="pf-stats"><div><dt>Active positions</dt><dd>{owner && !data.loading && !data.error ? active.length : '—'}</dd></div><div><dt>Ready to claim</dt><dd className="pf-yes">{canManage && !data.loading && !data.error ? claimable.length : '—'}</dd></div><div><dt>Closed positions</dt><dd>{owner && !data.loading && !data.error ? closed.length : '—'}</dd></div></dl>
      </section>
      ...
      {data.error && owner && <p className="pf-error" role="alert">{data.error} Use Refresh to try again.</p>}
      {incomplete && owner && <p className="pf-notice" role="status">Some market reads or historical records are unavailable or incomplete. Counts may be incomplete; refresh to retry.</p>}
      <div className={`pf-content ${selected ? 'has-action' : ''}`}>
        <section className="pf-list" aria-label={`${tab} positions`} aria-busy={data.loading}>
          {profile?.chain === 'solana' ? <div className="pf-empty"><h2>Solana profile is read-only for now</h2><p>This address is correctly scoped to Solana, but no Solana portfolio indexer is connected to this prediction venue yet.</p></div> : !owner ? <div className="pf-empty"><WalletCards size={32}/><h2>Make this portfolio yours.</h2><p>Log in with the wallet you used to trade. Your active positions, closed history, and available claims will appear here.</p>{walletControl}</div> : data.loading ? <div className="pf-loading" role="status">Loading on-chain positions…<div/><div/><div/></div> : data.error ? <div className="pf-empty"><h2>Portfolio temporarily unavailable</h2><p>We couldn’t load these positions. No empty balance has been assumed.</p></div> : visible.length === 0 ? <div className="pf-empty"><h2>{search ? 'No matching positions' : incomplete ? 'No positions in the available records' : tab === 'active' ? 'No active positions yet' : tab === 'orders' ? 'No remaining orders' : 'No closed positions yet'}</h2><p>{search ? 'Try another market name or match ID.' : tab === 'active' ? 'Holdings and unclaimed winnings appear here after trading.' : 'Fully exited markets and losing settled positions appear here.'}</p><a href="/">Explore the arena <ArrowUpRight size={15}/></a></div> : <div className="pf-table-scroll"><table><thead><tr><th>Market / outcome</th><th>{tab === 'orders' ? 'Remaining orders' : 'Shares'}</th><th>Status</th><th>Best bid</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map(row => {
