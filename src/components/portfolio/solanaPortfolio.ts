...
import { decodeMarket, decodePosition, decodeVault } from '../../../packages/adapters/solana/accounts'
import { positionAddress, TOKEN_PROGRAM_ID, vaultAddress, vaultCollateralAddress } from '../../../packages/adapters/solana/wire'
import { bindingAddress, bookAddress, claimMintAddress, decodeBinding, type Outcome } from '../../../packages/adapters/solana/manifest/wire'
...
export class ManifestPortfolioReader {
  constructor(private readonly adapter: ManifestAdapter, private readonly now: () => number = Date.now) {}
  ...
  async discover(owner: string, limit = 96): Promise<string[]> {
    ...
    const accounts = await connection.getParsedTokenAccountsByOwner(new PublicKey(owner), { programId: TOKEN_PROGRAM_ID }, 'confirmed')
    ...
    const infos = await connection.getMultipleAccountsInfo(mints.map(mint => new PublicKey(mint)), 'confirmed')
  ...
  async read(owner: string, marketIds: readonly string[], discovered: readonly string[] = []): Promise<ManifestPortfolio> {
    ...
    const headInfo = await connection.getMultipleAccountsInfo(head, 'confirmed')
    ...
    if (headInfo[0]) {
      ...
      } catch { /* A vault that cannot be decoded is reported as absent, never as zero collateral. */ }
    }
    ...
    for (let offset = 0; offset < keys.length; offset += BATCH_KEYS) {
      infos.push(...await connection.getMultipleAccountsInfo(keys.slice(offset, offset + BATCH_KEYS), 'confirmed'))
    }
    ...
    const holdings = marketIds.map((marketId, index) => {
      ...
      } catch { /* A position bound elsewhere is not this trader's; leave it out. */ }
      ...
      for (const outcome of [0, 1] as const) {
        ...
        } catch { /* One unreadable side must not blank the side that works. */ }
