...
function catalogueItem(row: unknown): CatalogueItem | null {
  ...
  return {
    ...
    tradeLocksAt: item.tradeLocksAt, outcomes: [outcomes[0]!, outcomes[1]!],
  }
}
...
/** Catalogue items in the shape the venue pipeline already consumes, with the
 *  market address derived here rather than accepted from the server. A resolved
 *  or cancelled item is not tradable and is not converted. */
export function catalogueQuestions(items: readonly CatalogueItem[], venue: PublicPredictionVenue | null | undefined): ReservedSolanaQuestion[] {
  if (!venue?.programId) return []
  const program = new PublicKey(venue.programId)
  const questions: ReservedSolanaQuestion[] = []
  for (const item of items) {
    if (item.status === 'resolved' || item.status === 'cancelled') continue
    // The pipeline's own vocabulary: an item that has opened trades as `live`,
    // one that has not yet is `reserved`.
    const status = item.status === 'scheduled' ? 'reserved' as const : 'live' as const
    questions.push({
      eventId: item.eventId, matchId: item.matchId, questionId: item.questionId,
      marketId: questionMarketAddress(program, hexBytes(item.matchId), hexBytes(item.questionId)).toBase58(),
      label: item.title, outcomes: [item.outcomes[0]!.id, item.outcomes[1]!.id],
