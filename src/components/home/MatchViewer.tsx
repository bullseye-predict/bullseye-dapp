    ...
    <div className={`ch-screen ${season ? 'is-season' : ''}`}>
      <TabPanel id="live" idPrefix="highlight-view" active={view === 'live'}>
        <div className="sh-broadcast" ref={frame}>
          ...
          {intermission ? <div className="ch-intermission" role="status"><div className="ch-intermission-copy"><span>5-MINUTE INTERMISSION</span><h2>Next match <b>{matchCode}</b></h2><div className="ch-intermission-time"><span>STARTS IN</span><strong>{remaining}</strong><small>{serverDeadline ? 'SERVER CLOCK' : 'SYNCING SERVER CLOCK'}</small></div><p>The room is reserved and all entrants are prepared. Trading can open before kickoff as soon as this network’s 12 event markets are provisioned.</p><strong>{match.roster.length} / 12 AGENTS CONFIRMED</strong></div><div className="ch-intermission-roster" aria-label="Next match agent roster">{match.roster.map((entry) => { const agent = snapshot.agents.find((item) => item.id === entry.agentId); return <div key={entry.agentId}><AgentPortrait number={agent?.number ?? Number(entry.agentId.split('-')[1])}/><span><strong>{entry.codename}</strong><small>{agent?.archetype ?? 'GENESIS AGENT'}</small></span></div> })}</div><small className="ch-intermission-lock">CHECK THE TRADE PANEL FOR THIS EVENT’S ON-CHAIN CUTOFF</small></div> : <div className="sh-broadcast-bottom"><div><span className="sh-map-label"><Crosshair size={14}/> COOLA / GENESIS SERIES</span><h2>{match.phase === 'settled' ? 'MATCH COMPLETE' : match.map}</h2><div className="ch-broadcast-roster">{match.roster.map((entry) => <span title={entry.codename} key={entry.agentId}><AgentPortrait number={Number(entry.agentId.split('-')[1])}/></span>)}<span>{match.roster.length} CAN AGENTS <span>/ {match.phase === 'settled' ? 'INTERMISSION' : match.mode}</span></span></div></div><div className="ch-broadcast-actions"><a href={liveHref}>Live arena <ArrowUpRight size={12}/></a><button className="sh-icon-button" onClick={fullscreen} aria-label="Full screen broadcast"><Maximize size={17}/></button></div></div>}
