...
export function OrderBookTable({ asks, bids, decimals, last, label }: { asks: DepthLevel[]; bids: DepthLevel[]; decimals: number; last?: number; label: string }) {
  ...
  return <table className="ch-order-book" aria-label={`${label} order book`}><caption className="sr-only">Asks above last trade, bids below. Depth and totals accumulate from the best price.</caption><colgroup><col className="ch-book-side-column"/><col/><col/><col/></colgroup><thead><tr><th scope="col"><span className="sr-only">Side</span></th><th scope="col">PRICE</th><th scope="col">SHARES</th><th scope="col">TOTAL</th></tr></thead><tbody>
    {rows(askRows, 'ask')}
    <tr className="ch-book-spread"><td colSpan={2}>Last: {last === undefined ? '—' : `${(last * 100).toLocaleString(undefined, { maximumFractionDigits: 2 })}¢`}</td><td colSpan={2}>Spread: {spread === undefined ? '—' : `${number(spread * 100n, decimals)}¢`}</td></tr>
    {rows(bidRows, 'bid')}
  </tbody></table>
}
...
export function LiveOrderBook({ market, isNo, label, collateral }: { market: ArenaMarket; isNo: boolean; label: string; collateral: string }) {
  ...
  return <div className="ch-live-book">
    <div className="ch-book-toolbar"><strong className={isNo ? 'is-no' : 'is-yes'}>{label} order book</strong><span>{collateral} total</span><button type="button" aria-label={`Refresh ${label} order book`} disabled={refreshing} onClick={() => refreshDreamDex(market.onchain!.chainId)}><RefreshCw size={15}/><span className="sr-only">Refresh</span></button></div>
    {data ? <OrderBookTable asks={(isNo ? data.book?.noAsks : data.book?.yesAsks) ?? []} bids={(isNo ? data.book?.noBids : data.book?.yesBids) ?? []} decimals={data.market.decimals} last={last} label={label}/> : <p className="ch-book-message" role="status">{error ?? 'Loading order book…'}</p>}
