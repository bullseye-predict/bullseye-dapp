...
export function InteractionConsole({ source, snapshot, match, market, outcome, onOutcome, section, onSection, promptAgentId, intermission, hideChat = false, hidePrompt = false, simulation = true, answer: externalAnswer, onAnswer }: Props) {
  ...
  return <aside className="ch-console" aria-label="Match interaction console">
    <ConsolePanel {...panel('trade')} title="Trade" icon={<ArrowUpRight size={16}/>} meta={simulation ? 'SIMULATION' : 'SIMULATION OFF'}><TradeTicket source={source} snapshot={snapshot} market={market} outcome={outcome} onOutcome={onOutcome} answer={answer} onAnswer={selectAnswer} simulation={simulation}/></ConsolePanel>
    <ConsolePanel {...panel('automate')} title="Auto trader" icon={<Bot size={16}/>} meta={`${rules.filter((rule) => rule.status === 'armed').length} ARMED`}>
      <p className="sh-console-intro">An agent watches your selected prediction.</p>
