...
import { GENESIS_TIER_COLOR, GENESIS_TIER_INK, genesisMint, mintSupplyText, supplyLabel } from './genesisMint'
  ...
  return <section className="sh-genesis" id="agents">
    ...
    <div className="sh-agent-grid">{snapshot.agents.map((agent) => {
      ...
      return <button className={`sh-agent-card ${selectedId === agent.id ? 'is-selected' : ''}`} key={agent.id} style={{ ...accentStyle(agent.color), ...tier }} onClick={(event) => { anchor.current = event.currentTarget; setSelectedId(selectedId === agent.id ? null : agent.id) }} aria-expanded={selectedId === agent.id} aria-haspopup="dialog" aria-controls={selectedId === agent.id ? `genesis-profile-${agent.id}` : undefined} aria-label={`View ${agent.codename}${agent.subname ? `, ${agent.subname}` : ''}, Genesis agent ${agent.number}${mint ? `, ${mint.rarity}, ${mintSupplyText(mint)}` : ''}`}>
        ...
        {/* A multi-part agent sells one supply per body, and their sum is a
            number nobody can buy. Its card therefore drops the single count
            beside the codename and lists each body's own supply instead. */}
        <div className={`sh-agent-card-name ${mint?.parts ? 'has-splits' : ''}`}>
          <h3>{agent.codename}</h3>
          {!mint?.parts && <b className="ga-card-supply">{mint ? supplyLabel(mint.supply) : '--'}</b>}
          ...
          {!mint?.parts && <span className={`ga-card-supply-label ${mint?.claim ? 'is-claim' : ''}`}>{mint?.claim ?? 'SUPPLY'}</span>}
          {mint?.parts && <ul className="ga-card-splits">{mint.parts.map((part) => <li key={part.label}>
            <span>{part.label}</span><b>{supplyLabel(part.supply)}</b><small>SUPPLY</small>
          </li>)}</ul>}
        </div>
      </button>
    })}</div>
