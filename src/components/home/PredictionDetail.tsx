...
export function PredictionDetail({ market, outcome, answer = 'yes', snapshot, referenceMarket, simulation, nested = false, onSelect }: Props) {
  ...
  return <div className="ch-topic-detail">
    <div className="ch-topic-tabs"><Tabs idPrefix={prefix} label={`${nested ? outcome.label : market.title} details`} value={tab} onChange={(value) => { setTab(value); onSelect(outcome, answer) }} tabs={[{ id: 'book', label: 'Order Book' }, { id: 'graph', label: 'Graph' }, { id: 'info', label: 'Info' }]}/><span>{simulation ? 'SAMPLE DATA' : 'NO LIVE ORDERS'}</span></div>
    <TabPanel id="book" idPrefix={prefix} active={tab === 'book'}>
      <div className="ch-book-heading"><label>Trade <select aria-label={`${nested ? outcome.label : market.title} order book outcome`} value={nested ? answer : outcome.id} onChange={(event) => nested ? onSelect(outcome, event.target.value as PredictionAnswer) : onSelect(market.outcomes.find((item) => item.id === event.target.value) ?? outcome)}>{nested ? <><option value="yes">Yes · {outcome.label}</option><option value="no">No · {outcome.label}</option></> : market.outcomes.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><span>COOLA / SHARE</span></div>
      {!book ? <div className="ch-market-empty" role="status"><strong>No orders yet.</strong><span>Quotes and depth appear only after live trading is connected and an order is placed.</span></div> :
