...
export function PredictionDetail({ market, outcome, answer = 'yes', snapshot, referenceMarket, simulation, nested = false, onSelect, collateral = 'COOLA' }: Props) {
  ...
  return <div className="ch-topic-detail">
    ...
    <TabPanel id="activity" idPrefix={prefix} active={tab === 'activity'} className="ch-topic-activity">
      {!market.onchain ? <div className="ch-market-empty"><strong>No on-chain activity yet.</strong><span>Opening this question will record its DreamDEX creation transaction here. User trades will appear as they are indexed.</span></div> : <OnchainActivity market={market} active={tab === 'activity'}/>}
    </TabPanel>
...
function OnchainActivity({ market, active }: { market: ArenaMarket; active: boolean }) {
  ...
  const explorer = market.onchain!.chainId === '50312' ? 'https://shannon-explorer.somnia.network/tx/' : 'https://explorer.somnia.network/tx/'
