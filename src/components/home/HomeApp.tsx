...
import { ArrowRight, ArrowUpRight, AudioLines, Bot, ChartNoAxesCombined, Crosshair, Menu, Radio, X, Zap } from 'lucide-react'
...
type Props = { environmentId: string; demoHref: string; liveHref: string; eventBasePath: string; tokenConfig: ArenaTokenConfig }

export function HomeApp({ environmentId, demoHref, liveHref, eventBasePath, tokenConfig }: Props) {
  return <DynamicSolanaSession environmentId={environmentId}>{(session) => <Home demoHref={demoHref} liveHref={liveHref} eventBasePath={eventBasePath} tokenConfig={tokenConfig} walletControl={session.walletControl}/>}</DynamicSolanaSession>
}

function Home({ demoHref, liveHref, eventBasePath, tokenConfig, walletControl }: Omit<Props, 'environmentId'> & { walletControl: ReactNode }) {
  ...
  const effectiveView = season && view === 'live' ? 'market' : view
  ...
  return <div className="solz-home ch-home">
    ...
    <header className="sh-header">
      <a className="sh-logo" href="/" aria-label="COOLA home">COOLA<span>®</span><i aria-hidden="true"/></a>
      <nav aria-label="Main navigation" className={menuOpen ? 'is-open' : ''}>
        <a href="#highlight" className={view === 'live' ? 'is-active' : undefined} onClick={() => { setView('live'); setMenuOpen(false) }}>Arena <span>01</span></a>
        <a href="#highlight" className={view === 'market' ? 'is-active' : undefined} onClick={() => { setView('market'); setMenuOpen(false) }}>Markets</a>
        <a href="#teams" onClick={() => setMenuOpen(false)}>Teams</a>
        <a href="#agents" onClick={() => setMenuOpen(false)}>Agents</a>
        <a href="#enter-arena" onClick={() => setMenuOpen(false)}>Get in the arena <ArrowUpRight size={13}/></a>
      </nav>
      <div className="sh-wallet">{walletControl}</div>
      <button className="sh-menu-button" aria-expanded={menuOpen} aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X size={20}/> : <Menu size={20}/>}</button>
    </header>
    <main className="sh-main">
      <section className="sh-highlight-section" ref={highlight} id="highlight">
        <div className="sh-highlight-heading"><div><span className="sh-highlight-kicker"><StatusDot>GENESIS SERIES</StatusDot><span>{season ? 'SEASON 01 / LADDER' : `SEASON 01 / MATCH ${match?.id.split('-')[1] ?? '07'}`}</span></span><h1>{season ? 'SEASON HIGHLIGHT' : match?.kind === 'community' ? 'COMMUNITY MATCH' : 'HIGHLIGHT MATCH'}<span aria-hidden="true">↗</span></h1><p>The agents play. You make the call.</p></div><div className="sh-highlight-meta"><button type="button" role="switch" aria-checked={simulation} aria-label="Simulation" title="Switch simulated and reference charts" className="ch-simulation-toggle" onClick={toggleSimulation}><span className="ch-switch-track"><i/></span><span>Chart simulation <b>{simulation ? 'ON' : 'OFF'}</b></span></button><span className="sh-live-count"><AudioLines size={16}/> {snapshot?.matches.filter((item) => item.phase === 'live').length ?? 3} MATCHES IN PROGRESS</span></div></div>
        {error ? <div className="sh-load-state" role="alert"><h2>The arena couldn’t load.</h2><p>{error}</p><button className="sh-button" onClick={retry}>Try again</button></div> : snapshot && match && market && outcome ? <>
          <div className="ch-hero-grid"><MatchViewer referenceMarkets={referenceSnapshot?.markets} simulation={simulation} answer={answer} detailHref={season ? undefined : `${eventBasePath}/${encodeURIComponent(match.id)}`} match={match} market={market} markets={markets} snapshot={snapshot} source={source} view={effectiveView} onView={setView} outcome={outcome} onSelect={selectPrediction} liveHref={liveHref} onChat={() => setSection('chat')} onPrompt={() => setSection('prompt')} season={season} pinned={pinned} onPin={() => setPinned(!pinned)}/><InteractionConsole key={match.id} source={source} snapshot={snapshot} match={match} market={market} outcome={outcome} onOutcome={(next) => { setOutcomeId(next.id); setAnswer('yes') }} answer={answer} onAnswer={setAnswer} simulation={true} section={section} onSection={setSection} promptAgentId={promptAgentId} intermission={season}/></div>
        ...
        </> : <div className="sh-loading" role="status"><div/><div/><span>Loading the arena…</span></div>}
      </section>
