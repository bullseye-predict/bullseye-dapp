...
import { createArenaFeed, currentMatchDrafts, type ArenaFeed } from './arenaFeed'
...
export function useHomeData(source: SolzDataSource, predictionApiUrl = '') {
  ...
  useEffect(() => {
    ...
    const render = (next: SolzSnapshot) => {
      base.current = next
      if (arena.current && events.current) setSnapshot(applyPredictionArena(next, arena.current, events.current))
    ...
    const refreshArena = async () => {
      ...
      try {
        const [nextArena, response] = await Promise.all([
          createArenaFeed('/api/agent-arena', fetch, predictionApiUrl)(controller.signal),
          fetch(new URL('/arena/events', predictionApiUrl), { signal: controller.signal, headers: { accept: 'application/json' } }),
        ])
        if (!response.ok) throw Error(`Prediction events could not load (${response.status}).`)
        const nextEvents = await response.json()
        if (!active) return
        arena.current = nextArena; events.current = nextEvents; setPredictionFeed(true)
        if (base.current) setSnapshot(applyPredictionArena(base.current, nextArena, nextEvents))
      } catch (reason) {
        ...
        try {
          const gameFeed = await createArenaFeed('/api/agent-arena', fetch, '', true)(controller.signal)
          if (!active) return
          arena.current = gameFeed
          events.current = currentMatchDrafts(gameFeed)
          setPredictionFeed(true)
          if (base.current) setSnapshot(applyPredictionArena(base.current, gameFeed, events.current))
          setError('')
        } catch {
          if (active) setError(reason instanceof Error ? reason.message : 'Prediction event catalogue could not load.')
        }
      } finally {
        if (active && predictionApiUrl) timer = setTimeout(() => void refreshArena(), 15_000)
      }
    }
    setError('')
  ...
  return { snapshot, referenceSnapshot, error, predictionFeed, retry: () => setAttempt((value) => value + 1) }
}
