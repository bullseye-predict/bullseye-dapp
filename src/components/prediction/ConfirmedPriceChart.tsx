...
export function ConfirmedPriceChart({ candles, label, sourceLabel = 'confirmed trades' }: { candles: Candle[]; label: string; sourceLabel?: string }) {
  const first = candles[0], last = candles.at(-1)
  if (!first || !last) return <div className="pt-empty pt-chart-empty"><strong>No {sourceLabel} yet</strong><p>The chart begins when the first trade settles.</p></div>
  ...
  return <figure className="pt-chart"><figcaption><span>{label} · {sourceLabel}</span><strong>{priceLabel(last.close)}</strong></figcaption><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${label}: ${candles.length} trade intervals, last price ${priceLabel(last.close)}`}>
