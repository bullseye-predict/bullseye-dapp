...
export function EventMarketRail({ snapshot, match, paths, variant, predictionId }: { snapshot: SolzSnapshot; match: SolzMatch; paths: EventPaths; variant: EventVariant; predictionId?: string }) {
  ...
  return <nav className="ev-market-nav" aria-label="Arena event navigation">
    ...
    <div className="ev-nav-matches">{snapshot.matches.filter((item) => item.phase === 'live' || item.id === match.id).map((item) => <a href={eventHref(paths.variants[variant], item.id)} aria-current={item.id === match.id ? 'page' : undefined} key={item.id}><span className="ev-nav-marks">{item.teams.slice(0, 2).map((team) => <TeamMark key={team.teamId} id={team.teamId} color={team.color}/>)}</span><span><strong>{item.teams.map((team) => team.symbol).join(' / ')}</strong><small>{item.mode} · {item.map}</small></span><ChevronRight size={13}/></a>)}</div>
