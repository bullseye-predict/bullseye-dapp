...
export class HttpPredictionVenue implements PredictionVenue {
  ...
  synchronizeClock(timestamp: number): void {
    if (!Number.isSafeInteger(timestamp) || timestamp < 0)
      throw new PredictionError(
        "INVALID_CLOCK",
        "Invalid prediction server clock.",
      );
  ...
  protected async request<T>(
    ...
    const url = predictionUrl(path, this.options.baseUrl);
    ...
    if (authorize) {
      const proof = {
        ...
        expiresAt: this.currentTime() + 30_000,
      };
      ...
      headers = {
        ...
        ...proofHeaders({
          ...proof,
          signature: await this.options.signRequest(message),
        }),
      };
    }
    ...
    const payload = parsePredictionResponse<
      T & { code?: string; message?: string }
    >(await response.text());
    if (!response.ok)
      throw new PredictionError(
        payload.code ?? "VENUE_ERROR",
        payload.message ?? `Venue returned HTTP ${response.status}.`,
        response.status,
      );
    return payload;
  }
  ...
  private checkScope(market: Market, id?: string): void {
    if (
      ...
      (id !== undefined && market.id !== id)
    )
      throw new PredictionError(
        "VENUE_MISMATCH",
        "Server returned a market outside the selected venue.",
      );
  }
  async getMarket(id: string): Promise<Market> {
    const market = await this.request<Market>(
      `/markets/${encodeURIComponent(id)}`,
    );
    validateMarket(market);
    this.checkScope(market, id);
    return market;
  }
  getOrderBook(id: string, outcomeId?: number): Promise<OrderBook> {
    return this.request(
      `/markets/${encodeURIComponent(id)}/orderbook${outcomeId === undefined ? "" : `?outcomeId=${outcomeId}`}`,
    );
  }
  getPortfolioPositions(account: string): Promise<PortfolioPosition[]> {
    return this.request(`/users/${encodeURIComponent(account)}/positions`);
  }
  async getPositions(account: string): Promise<Position[]> {
    ...
    if (
      positions.some(
        (position) =>
          position.accountingComplete === false ||
          typeof position.costBasis !== "bigint" ||
          typeof position.realizedPnl !== "bigint",
      )
    )
      throw new PredictionError(
        "ACCOUNTING_UNAVAILABLE",
        "Complete position cost and realized PnL are required for automated trading.",
        503,
      );
  ...
  getOpenOrders(account: string, marketId?: string): Promise<Order[]> {
    return this.request(
      `/users/${encodeURIComponent(account)}/orders${marketId === undefined ? "" : `?marketId=${encodeURIComponent(marketId)}`}`,
    );
  }
  async placeOrder(input: PlaceOrderInput): Promise<OrderResult> {
    ...
    if (
      ...
      throw new PredictionError(
        "SIGNER_MISMATCH",
        "Signer changed the requested order.",
      );
  ...
  cancelAllOrders(marketId?: string): Promise<TxResult> {
    return this.request("/orders/cancel-all", "POST", { marketId }, true);
  }
  redeem(marketId: string): Promise<TxResult> {
    return this.options.redeem(marketId);
  }
  getBalance(account: string): Promise<Balance> {
    return this.request(`/users/${encodeURIComponent(account)}/balance`);
  }
}
