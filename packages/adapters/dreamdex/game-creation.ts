...
// Selector and codec verified against the deployed Shannon implementation.
...
export function gameQuestionDefinition(game: GameQuestion): QuestionDefinitionInput {
  ...
  if (![game.tradingStartsAt, game.tradingLocksAt, game.resolutionAt].every(value => Number.isSafeInteger(value) && value % 1000 === 0) || game.tradingStartsAt >= game.tradingLocksAt || game.tradingLocksAt >= game.resolutionAt) throw Error('Invalid game creation window.')
  return {
    ...
    resolutionTime: BigInt(game.resolutionAt / 1000),
    minAgreement: 1n,
    subcommitteeSize: 3n,
...
export function gameCreationArgs(game: GameQuestion, operatorId: number, venueId: Hex, addresses: { oracleHub?: Address; collateral?: Address }) {
  ...
  return [operatorId, venueId, addresses.oracleHub, gameQuestionDefinition(game), {
    oracleQuestionId: 0n,
    oracleAdapter: addresses.oracleHub,
    collateral: addresses.collateral,
    book: { tickSize: 1000n, minQuantity: 1_000_000n, lotSize: 1_000_000n },
    asset: game.agentId,
    strike: 0n,
    tradingStart: BigInt(game.tradingStartsAt / 1000),
    expiry: BigInt(game.tradingLocksAt / 1000),
    settlementWindow: 3600n,
    question: game.label,
    referenceQuestionId: 0n,
...
/** Admin sponsors creation only. User approvals, orders, and positions stay in user wallets. */
export class DreamDexGameCreator {
  ...
  async create(game: GameQuestion, beforeSend: () => void | Promise<void>, submitted: (hash: Hex) => void | Promise<void>) {
  ...
  async confirm(game: GameQuestion, hash: Hex): Promise<DreamDexPublicConfig['markets'][number]> {
    ...
    for (const log of receipt.logs) {
      ...
      if (args.operatorId !== this.operatorId || args.venueId !== this.venueId || args.question !== game.label || args.asset !== game.agentId || args.outcomeSlotCount !== 2 || args.voidPolicy !== 0) throw Error('Creation receipt does not match the game request.')
      const market = { eventId: game.eventId, questionId: `winner-${game.agentId}`, subjectId: game.agentId, label: game.label, marketId: args.marketId, oracleQuestionId: args.oracleQuestionId.toString(), tradingStartsAt: Number(args.tradingStart) * 1000, tradingLocksAt: Number(args.expiry) * 1000, voidPolicy: 0 as const }
      await this.resources.reader.inspect({ venue: 'DREAMDEX', chainId: '50312', matchId: game.eventId, ...market, oracleQuestionId: args.oracleQuestionId })
      return market
    }
